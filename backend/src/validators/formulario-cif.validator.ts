...
const categoriaCIFSchema = z.enum(
  ...
  {
    error:
      'Categoria deve ser uma das seguintes: ESTRUTURA, FUNCAO, ATIVIDADE_PARTICIPACAO, FACTOR_AMBIENTAL',
  }
)
...
const itemCIFSchema = z.object({
  ...
  nivel: z
    ...
    .int()
    .min(0, 'Nível do item CIF deve ser um número inteiro não negativo')
    .optional(),
  qualificador1: z
    .number({ error: 'Qualificador 1 do item CIF deve ser um número' })
    .int()
    .min(0, 'Qualificador 1 do item CIF deve ser um número inteiro não negativo')
    .optional(),
  tipoQualificador1: tipoFactorAmbientalSchema.optional(),
  qualificador2: z
    .number({ error: 'Qualificador 2 do item CIF deve ser um número' })
    .int()
    .min(0, 'Qualificador 2 do item CIF deve ser um número inteiro não negativo')
    .optional(),
  qualificador3: z
    .number({ error: 'Qualificador 3 do item CIF deve ser um número' })
    .int()
    .min(0, 'Qualificador 3 do item CIF deve ser um número inteiro não negativo')
    .optional(),
  qualificador4: z
    .number({ error: 'Qualificador 4 do item CIF deve ser um número' })
    .int()
    .min(0, 'Qualificador 4 do item CIF deve ser um número inteiro não negativo')
    .optional(),
