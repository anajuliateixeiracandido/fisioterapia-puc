...
const cadastroRelatorioSchema = z.object({
  pacienteId: z
    .number()
    .int()
    .positive({ message: 'ID do paciente deve ser um número inteiro positivo' }),
  professorResponsavelId: z
    .number()
    .int()
    .positive({ message: 'ID do professor responsável deve ser um número inteiro positivo' })
    .optional(),
  formularioCIF: cadastroFormularioCIFSchema,
})

const editarRelatorioSchema = z
  .object({
